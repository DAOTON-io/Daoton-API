...
app.get('/getDAOs', async (req, res) => {
    ...
    result = await dbo.collection("DAOs").find({}).toArray();
    res.json(result);
    db.close();
});
...
app.get('/getContracts/:id', async (req, res) => {
    db= await MongoClient.connect(url)
    var dbo = db.db("DAOTON");
    result = await dbo.collection("Contracts").find({DAO_Id: req.params.id}).toArray()
    res.json(result);
    db.close();
});
...
app.post('/saveDAO', async (req, res) => {
    ...
    db = await MongoClient.connect(url)
    var dbo = db.db("DAOTON");
    var sender = decoded.address;
    var myobj = { DAO_Name: req.body.DAO_Name, Creator: sender, DAO_Description: req.body.DAO_Description, DAO_Address: req.body.DAO_Address, DAO_Logo: req.body.DAO_Logo, DAO_Website: req.body.DAO_Website, DAO_Token_Address : req.body.DAO_Token_Address, DAO_Token_Symbol : req.body.DAO_Token_Symbol};
